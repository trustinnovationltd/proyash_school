...
@section('title', __('messages.designation_list'))
    ...
    <div class="row">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
                        <h3 class="card-title">{{ __('messages.designation_list') }}</h3>
                    </div>
                    <div class="col-md-6" style="text-align: right">
                        <a href="{{ route('designations.create') }}" type="button" class="btn btn-success btn-sm"
                            style="color: white">
                            {{ __('messages.add_designation') }}
                        </a>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @foreach ($designations as $item)
                            <tr class="align-middle">
                                ...
                                <td>
                                    <a class="btn btn-sm btn-info p-2" href="{{ route('designations.edit', $item->id) }}"
                                        title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                    <a class="btn btn-sm btn-danger p-2" href="#"
                                        onclick="confirmDelete(event, '{{ $item->id }}')" title="Delete"><i
                                            class="fa-solid fa-trash"></i></a>

                                    <form id="designation-delete-form-{{ $item->id }}"
        ...
        function confirmDelete(event, id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('designation-delete-form-' + id).submit();
                }
            });
        }
    </script>
