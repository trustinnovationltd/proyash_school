...
@section('title', __('messages.section_list'))
    ...
    <div class="row">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
                        <h3 class="card-title">{{ __('messages.section_list') }}</h3>
                    </div>
                    <div class="col-md-6" style="text-align: right">
                        <a href="{{ route('sections.create') }}" type="button" class="btn btn-success btn-sm"
                            style="color: white">
                    ...
                    <tbody>
                        @foreach ($sections as $item)
                            <tr class="align-middle">
                                ...
                                <td>
                                    <a class="btn btn-sm btn-info p-2" href="{{ route('sections.edit', $item->id) }}"
                                        title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                    <a class="btn btn-sm btn-danger p-2" href="#"
                                        onclick="confirmDelete(event, '{{ $item->id }}')" title="Delete"><i
                                            class="fa-solid fa-trash"></i></a>

                                    <form id="section-delete-form-{{ $item->id }}"
                                        action="{{ route('sections.destroy', $item->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
        ...
        function confirmDelete(event, id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('section-delete-form-' + id).submit();
                }
            });
        }
    </script>
