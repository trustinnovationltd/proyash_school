...
@section('title', __('messages.class_list'))
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-md-6" style="text-align: right">
                        <a href="{{ route('classes.create') }}" type="button" class="btn btn-success btn-sm"
                            style="color: white">
                    ...
                    <tbody>
                        @foreach ($classes as $item)
                            <tr class="align-middle">
                                ...
                                <td>
                                    <a class="btn btn-sm btn-info p-2" href="{{ route('classes.edit', $item->id) }}"
                                        title="Edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                    <a class="btn btn-sm btn-danger p-2" href="#"
                                        onclick="confirmDelete(event, '{{ $item->id }}')" title="Delete"><i
                                            class="fa-solid fa-trash"></i></a>

                                    <form id="class-delete-form-{{ $item->id }}"
                                        action="{{ route('classes.destroy', $item->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
        ...
        function confirmDelete(event, id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('class-delete-form-' + id).submit();
                }
            });
        }
    </script>
